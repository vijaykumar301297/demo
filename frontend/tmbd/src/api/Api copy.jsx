...
export default function Api() {
    ...
    const runtime = movieData?.Search[0]?.Runtime;
    const totalMinutes = parseInt(runtime, 10); 
    const hours = Math.floor(totalMinutes / 60); 
    const minutes = totalMinutes % 60;
    const releaseDate = movieData?.Search[0]?.Released; 
    const formattedDate = formatReleaseDate(releaseDate);

    return (
        <div className='main' style={{color:'black'}}>
            <div className="img_block">
                <img src={movieData?.Search[0]?.Poster} alt={`${movieData?.Search[0]?.Title} image`} />
            </div>
            <div>
                <h1>{movieData?.Search[0]?.Title} ({movieData?.Search[0]?.Year})</h1>
                <div className="text">
                    <p className='rated'>{movieData?.Search[0]?.Rated}</p>
                    <p>{`${formattedDate} (IN)`}</p>
                    <p>{`${movieData?.Search[0]?.Genre}`}</p>
                    <p>{`${hours}h ${minutes}m`}</p> 
                </div>
                <div>
                    <div style={{ display: 'flex', justifyContent: 'flex-start', alignItems: 'center', gap: '2rem', fontSize: '1.6rem' }}>
                        <p style={{ display: 'flex', justifyContent: 'flex-start', alignItems: 'center' }}>
                            ...
                            <span style={{ fontSize: '1.4rem' }}>User <br /> Score</span> 
                        </p>
                    </div>
                    ...
                    <p className='vibe'>
                        Whats your vibe? 
                        <span title='Login to use TMDB rating'>
                            <IoInformationCircleOutline />
                        </span>
                    </p>

                    <div style={{ display: 'flex', justifyContent: 'flex-start', alignItems: 'center', gap: '2rem', fontSize: '1.6rem' }}>
                        <p title="Login to create and edit list" style={{ padding: '1rem', borderRadius: '100%', background: '#032541', color: 'white' }}>
                            <BsCreditCardFill />
                        </p>
                        <p style={{ padding: '1rem', borderRadius: '100%', background: '#032541', color: 'white' }}>
                            <FaHeart />
                        </p>
                        <p><FaPlay /> Play Trailer</p>
                    </div>
                    ...
                    <div style={{ fontSize: '1.4rem', margin: '1rem 0' }}>
                        <p style={{ fontWeight: '600' }}>Obviously.</p>
                        <p style={{ fontWeight: '600' }}>Overview</p>
                        <p style={{ fontSize: '1rem', letterSpacing: '1px' }}>{movieData?.Plot}</p>
                        <div>
                            <p>{movieData?.Director}</p>
                            <p style={{ fontWeight: '600' }}>Director, Writer</p>
                        </div>
